...
hover = HoverTool(
    tooltips=[("Title", "@title"), ("Author", "@author"), ("Year", "@year"),]
)

fig = figure(
    tools=[hover, "zoom_in", "zoom_out", "wheel_zoom", "pan", "undo", "redo", "help"],
    ...
    if "geoLocations" in metadata:
        ...
        for g in geo:
            ...
            if "geoLocationPoint" in g:
                ...
                tlon, tlat = transformer.transform(
...
segment = Segment(x0="x0", y0="y0", x1="x1", y1="y1", line_color="color", line_width=3)
fig.add_glyph(source, segment)

circle = Circle(x="pt_lon", y="pt_lat", size=6, fill_color="color")
nonselection_circle = Circle(
    x="pt_lon", y="pt_lat", size=6, fill_color="color", fill_alpha=0.8
)
fig.add_glyph(
    source, circle, selection_glyph=circle, nonselection_glyph=nonselection_circle
)
